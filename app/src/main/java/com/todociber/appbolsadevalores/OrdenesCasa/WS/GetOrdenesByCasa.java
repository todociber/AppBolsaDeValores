 ...
 */
public class GetOrdenesByCasa {

    BufferedReader in = null;

    public int  error,erroCode,result;
    public String Email,Password,TokenPush;
    public Context context;

    public GetOrdenesByCasa(Context context, String token, String idCliente, String idCasa) {

        String urlBase = context.getResources().getString(R.string.urlSistemaASI);
        String  url = urlBase+"api/getOrdenesByClienteCasa/"+idCliente+"/"+idCasa+"?token="+token;
        ...
        try {
            ...
            if (error == 0) {
            JSONArray jsdonData = jsonObject.getJSONArray("data");
            ProcesarOrdenesByCasa procesarOrdenesByCasa = new ProcesarOrdenesByCasa(context);
                procesarOrdenesByCasa.procesarOrden(jsdonData);
